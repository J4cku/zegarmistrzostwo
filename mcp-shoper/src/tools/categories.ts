...
export function registerCategoryTools(server: McpServer) {
  ...
  server.tool(
    "shoper_create_category",
    "Create a new category. Translations object keys are language IDs. Required translation fields: name, active ('1'/'0').",
    {
  ...
  server.tool(
    "shoper_update_category",
    "Update an existing category by ID. Pass only the fields you want to change.",
    {
  ...
  server.tool(
    "shoper_delete_category",
    "Delete a category by ID. Products must be removed or reassigned first.",
    { category_id: z.number().int().positive().describe("Category ID to delete") },
    async ({ category_id }) => {
      await shoperFetch(`/categories/${category_id}`, { method: "DELETE" });
      return { content: [{ type: "text" as const, text: JSON.stringify({ deleted: true, category_id }) }] };
    },
  );
}
