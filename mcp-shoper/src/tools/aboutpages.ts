...
export function registerAboutPageTools(server: McpServer) {
  server.tool(
    "shoper_list_aboutpages",
    "List about/info pages with pagination",
  ...
  server.tool(
    "shoper_get_aboutpage",
    "Get a single about page by ID",
    { page_id: z.number().int().positive().describe("About page ID") },
    async ({ page_id }) => {
  ...
  server.tool(
    "shoper_update_aboutpage",
    "Update an existing about page by ID. Fields: name, content (HTML), active (0/1), lang_id, seo_title, seo_description, seo_url.",
    {
