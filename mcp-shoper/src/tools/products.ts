...
export function registerProductTools(server: McpServer) {
  ...
  server.tool(
    "shoper_create_product",
    "Create a new product. Translations object keys are language IDs (e.g. '1' for Polish). Required translation fields: name, active ('1'/'0'). Optional: short_description, description, seo_title, seo_description.",
    {
      stock: z.object({
  ...
  server.tool(
    "shoper_update_product",
    "Update an existing product by ID. Pass only the fields you want to change.",
    {
  ...
  server.tool(
    "shoper_delete_product",
    "Delete a product by ID. Also removes associated images and stock.",
    { product_id: z.number().int().positive().describe("Product ID to delete") },
    async ({ product_id }) => {
      await shoperFetch(`/products/${product_id}`, { method: "DELETE" });
      return { content: [{ type: "text" as const, text: JSON.stringify({ deleted: true, product_id }) }] };
    },
  );
}
